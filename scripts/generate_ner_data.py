...
def load_data(csv_path: str) -> pd.DataFrame:
    """Load the Telegram data from CSV file."""
    try:
...
def extract_entities(text: str) -> List[Tuple[str, str]]:
    ...
    price_patterns = [
        (r'(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*ብር\b', 'B-PRICE', 'I-PRICE'),  # 1,200 ብር
        (r'ዋጋ[\s:፡]*\s*(\d+)\s*ብር', 'B-PRICE', 'I-PRICE'),  # ዋጋ፡ 1200 ብር
        (r'(\d+)\s*ብር', 'B-PRICE', 'I-PRICE'),  # 1200ብር
    ]
...
if __name__ == "__main__":
    # Configuration
    CSV_PATH = "data/processed/cleaned_telegram_data.csv"
